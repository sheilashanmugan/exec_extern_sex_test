...
data18sub <- subset(subj.data, select=c("bblid", "scanid", "sex1m1", "fedu1","medu1", "ageInYears", "nbackRelMeanRMSMotion", "nbackBehAllDprime", "cope.path.2m0", "varcope.path.2m0"))
mdata <- melt(data18sub, id=c("bblid","scanid", "sex1m1", "ageInYears", "nbackRelMeanRMSMotion","nbackBehAllDprime", "cope.path.2m0", "varcope.path.2m0"))
mdata$psex <- ifelse(mdata$variable=="fedu1", 1,-1)
mdata$peduYears <-mdata$value
mdata$categorical_include <- "1"


mdata$ageInYearsDemean <- (mdata$ageInYears - mean(mdata$ageInYears))
#mdata$sex1m1Demean <- (mdata$sex1m1 - mean(mdata$sex1m1))
#mdata$medu1Demean <- (mdata$medu1 - mean(mdata$medu1))
#mdata$fedu1Demean <- (mdata$fedu1 - mean(mdata$fedu1))
mdata$psexDemean <- (mdata$psex - mean(mdata$psex))
mdata$peduYearsDemean <- (mdata$peduYears - mean(mdata$peduYears))
mdata$nbackRelMeanRMSMotionDemean <- (mdata$nbackRelMeanRMSMotion - mean(mdata$nbackRelMeanRMSMotion))
mdata$nbackBehAllDprimeDemean <- (mdata$nbackBehAllDprime - mean(mdata$nbackBehAllDprime))




###ALL COVARIATES
variables<-c( "sex1m1", "psex", "peduYearsDemean", "ageInYearsDemean", "nbackRelMeanRMSMotionDemean") 
lin.mod<-"~   sex1m1*psex*peduYearsDemean + ageInYearsDemean + nbackRelMeanRMSMotionDemean" 
int <- "sexXpsex*peduYearsDemean"
###
