...
data <- read.csv("/Users/sheilash/Desktop/projects/exec_extern_sex/inputData/data_agg/connectivity/nbackNetworks_264PowerPNC_dataagg_20180913.csv")
data2 <- data
...
for (i in 1:length(namelist)){
  yvar <- namelist[i]
  print(yvar)
  newdata3 <- subset(data2, select=c("bblid", yvar, "sex01","ageInYears", "nbackRelMeanRMSMotion", "externalizing_corrtraitsv2"))
  ...
  volume_estimate <- function(data, indices){
    ...
    full.model <- lm(yvar ~ sex01+ ageInYears + nbackRelMeanRMSMotion, data=newdata)
    summary(full.model)
    ##save parameter a
    ...
    full.model <- lm(externalizing_corrtraitsv2 ~ yvar + sex01   + ageInYears + nbackRelMeanRMSMotion, data=newdata)
    summary(full.model)
    ##save parameter b
...
########## 
yvar <- "between_overall"
print(yvar)
newdata3 <- subset(data2, select=c("bblid", yvar, "sex01","ageInYears", "nbackRelMeanRMSMotion", "externalizing_4factorv2"))
newdata3["yvar"] <- (newdata3[,2])
newdata3 <- newdata3[complete.cases(newdata3), ]
...
volume_estimate <- function(data, indices){
  ...
  ##Step 2: fit the model to estimate the assoc between the outcome (externalizing symp) and  the mediator (yvar), adjusted for sex
  full.model <- lm(externalizing_4factorv2 ~ yvar + sex01   + ageInYears + nbackRelMeanRMSMotion, data=newdata)
  summary(full.model)
  ##save parameter b
  b <- summary(full.model)$coefficients[2,"Estimate"]
  ##save c' as the regression coeff for sex01
...
results_med = boot(data = newdata3, statistic = volume_estimate, R = 2000)
results_med
full.model <- lm(yvar ~ sex01+ ageInYears + nbackRelMeanRMSMotion, data=newdata3)
