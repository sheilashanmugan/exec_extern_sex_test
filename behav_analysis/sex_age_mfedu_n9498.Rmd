...
```{r include=FALSE}
data <- read.csv("/data/jux/BBL/projects/exec_extern_sex/inputData/data_agg/n6469_from9498_20180829.csv")
...
data1 <- subset(data, select=c("bblid", yvar, "ageInYears", "sex01", "medu1"))
data1["yvar"] <- data1[,2]
data1<- subset(data1, yvar != "NA")
data1<- subset(data1, medu1 != "NA") #n=1138
```

#sex x maternal education interaction on externalizing symptoms,  
Using bifactor scores. Interaction not seen in img sample, medu -> fewer symptoms
```{r echo=FALSE}
...
```{r include=FALSE}
yvar="externalizing_4factorv2"
data1 <- subset(data, select=c("bblid", yvar, "ageInYears", "sex01", "fedu1"))
data1["yvar"] <- data1[,2]
data1<- subset(data1, yvar != "NA")
