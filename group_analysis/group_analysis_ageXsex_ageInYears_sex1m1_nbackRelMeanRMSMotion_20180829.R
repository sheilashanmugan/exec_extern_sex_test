...
###ALL COVARIATES
variables<-c("ageInYears", "sex1m1", "nbackRelMeanRMSMotion") 
lin.mod<-"~  ageInYears*sex1m1 + ageInYears + sex1m1 + nbackRelMeanRMSMotion" 
int <- "ageInYearsXsex1m1"
###
...
#subj.data$sex1m1 <- as.factor(subj.data$sex1m1) #doing this reverts it back to 0,1 when calculating design matrix below (t X <- model.matrix)
...
#######################
file.copy("/data/jux/BBL/projects/exec_extern_sex/scripts/group_analysis/group_analysis_ageXsex_ageInYears_sex1m1_nbackRelMeanRMSMotion_20180829.R",resultsdir)
