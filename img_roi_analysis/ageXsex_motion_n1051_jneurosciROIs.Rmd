---
title: "Age x sex Effect on jneurosci ROIs n1051"
...
namelist <- c(list2x)
list1a <- c("age", "sex1m1")
list1b <- c("motion", "ageXsex1m1") #interaction last here
...
for (i in 1:length(namelist)){
  yvar <- namelist[i]
  newdata <- subset(data3, select=c("scanid", "ageInYears", "sex1m1", yvar, "nbackRelMeanRMSMotion"))
  newdata["yvar"] <- (newdata[,4])
  newdata <- subset(newdata, yvar != "NA")
  
  try(full.model <- lm(yvar ~ ageInYears*sex1m1 + ageInYears +nbackRelMeanRMSMotion,  data=newdata))
