...
###ALL COVARIATES
variables<-c( "medu1_demean", "ageInYears_demean", "nbackRelMeanRMSMotion_demean") 
lin.mod<-"~   medu1_demean + ageInYears_demean + nbackRelMeanRMSMotion_demean" 
subgrp <- "femaleOnly"
###
...
resultsdir<-paste(outdir,paste(subgrp,variables_collapsed,sep="_"),sep="/")    #resutls dir is the subdirectory where the model and stats are saved; this can vary by model chosen
dir.create(outdir)
...
#######################
file.copy("/data/jux/BBL/projects/exec_extern_sex/scripts/group_analysis/group_analysis_age_sex_mfedu_20180829.R",resultsdir)
